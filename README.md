...
- **Film grain** — a pre-generated 256×256 noise canvas is tiled over the full physical canvas at a random offset using `source-over` blend. Tiling coordinates use `canvas.width`/`canvas.height` (physical pixels) at the identity transform — dividing by DPR would only cover the top-left quarter on Retina screens, leaving a hard seam boundary that flickers each update. Offset advances every 4th frame (~15 fps), below the flicker-fusion threshold. Subtly stronger at night
...
5. **WebGL2 bloom pass** - main canvas uploaded as texture to an offscreen WebGL2 canvas; bright-extract → 4× Gaussian blur at ¼ res → temporal EMA blend (τ ≈ 9 frames) → composited onto main canvas with additive `'lighter'` blend
6. **Film grain pass** - 256×256 tiled noise canvas covering the full physical canvas (`canvas.width`/`canvas.height`, not divided by DPR), `source-over` blend at ~2.5–4.5% opacity, offset advanced every 4th frame
...
- **Bloom early-out** - the WebGL2 bloom pass skips all shader passes when `nightAlpha` is below 12% of dusk threshold, clearing the overlay to transparent instead. FBOs are allocated at ¼ screen resolution to keep GPU memory and fill-rate cost minimal
