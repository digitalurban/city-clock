...
- **Chimney smoke** - soft grey particles drift upward from residential chimneys. Emission rate responds to the season (more smoke in cold months Oct–Mar) and time of day (morning warm-up and evening heating peaks produce denser plumes). Canalside factories have a separate heavier smoke system — see *Factory chimney smoke* below
...
- **Canalside factories** - Northern UK-style mill buildings line the canal with varied character: three brick palettes (dark Victorian red, sooty weathered, orange-red newer brick), three roof styles (horizontal sawtooth northlight, vertical sawtooth, monitor/clerestory ridge), three chimney configurations (one large stack, two medium, three small), varying building heights with cobbled goods yards below shorter buildings, optional stone office/counting-house annexes, and exterior cast-iron drainpipes on some walls. Each factory has 1–2 loading bay doors on the canal-facing wall with horizontal roller rails. At night a sparse scatter of amber industrial windows lit at shift hours (more active 10pm–6am when night shifts run)
- **Factory goods & canal trade** - each factory yard gradually accumulates cargo (wooden crates, metal drums, canvas sacks) over time. When a narrowboat passes the loading bay, it slows and moors briefly while goods are transferred to its deck — visible as a tarpaulin-covered stack of crates. The cargo fades slowly as the boat continues on its way. A city information stream toast announces each loading event
- **Factory chimney smoke** - each factory independently cycles between active (30–120 s of warm grey smoke rising from its stacks) and idle (10–45 s quiet, for shift changes, stoking, or maintenance). Timers are staggered so factories along the canal stop and start independently rather than in unison. Smoke particles are larger and denser than residential chimney smoke
...
- **Canal corridor**: the rightmost column is a canal — Northern UK-style red-brick factories of varying height occupy most of the inner portion (with occasional canal-side parks), a stone towpath runs beside the water, and horizontal roads bridge over the canal at each grid row. Narrowboats navigate the water, slow near bridge crossings, and stop to collect goods from factory yards
